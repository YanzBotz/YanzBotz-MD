...
const makeMessagesSocket = (config) => {
    ...
    const patchMessageRequiresBeforeSending = (msg, recipientJids) => {
      if (msg && msg.deviceSentMessage && msg.deviceSentMessage.message && msg.deviceSentMessage.message.listMessage) {
        msg = JSON.parse(JSON.stringify(msg));
        msg.deviceSentMessage.message.listMessage.listType = WAProto_1.proto.Message.ListMessage.ListType.SINGLE_SELECT;
      }

      if (msg && msg.listMessage) {
        msg = JSON.parse(JSON.stringify(msg));
    ...
    const createParticipantNodes = async (jids, message, extraAttrs) => {
        ...
        const bytes = Utils_1.encodeWAMessage(requiredPatched)
